...
class AsmStringBuilder(private val commentSymbol: String) {
    ...
    fun addLine(
        ...
        op?.let { addOp(op) }
        if (reg1 != null) addRegs(reg1, reg2) else add("", REGS_OFFSET)
        comment?.let { addComment(comment) }
        newline()
        return this
    }
